...
const loginWithEmailNPasswordApi = async ({email, password}) => {
  try {
...
const signupCall = async ({role}) => {
  try {
    ...
    const signupAction = await fetch (process.env.REACT_APP_USERS_SIGNUP, {
      method: 'POST',
      body: JSON.stringify ({
        email,
        role,
    ...
    const signupJson = await signupAction.json ();
    return {
